...
class QuestionActivity : AppCompatActivity() {
    ...
    val handlerProgressDialog = object:Handler(Looper.getMainLooper()){
        override fun handleMessage(msg : Message){
            //value하나 증가시키고 100보다 작으면 대화상자에 값을 적용하고
            //자신에게 메시지를 다시 전송
            //value 가 100보다 크거나 같으면 대화상자 종료
            value ++
            Thread.sleep(100)
            if(value < 100){
    ...
    val handlerProgressBar = object:Handler(Looper.getMainLooper()){
        override fun handleMessage(msg : Message){
            //value하나 증가시키고 100보다 작으면 ProgressBar에 값을 적용하고
            //자신에게 메시지를 다시 전송
            //value 가 100보다 크거나 같으면 종료
            value ++
            Thread.sleep(100)
            if(value < 100){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnQuestion.setOnClickListener{
            AlertDialog.Builder(this)
                ...
                //이런 식으로 작업하면 작업이 끝날때까지 대화상자가 닫히지 않음
                .setPositiveButton("예",
